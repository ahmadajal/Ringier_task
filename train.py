...
logging.basicConfig(filename="train_log.log", encoding="utf-8", level=logging.DEBUG)
...
class Training:
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            acc_epoch = self.evaluate(model, test_dataloader)
            logging.info(f"Validation accuracy after epoch {epoch}: {acc_epoch}")
        logging.info("Training finished!")
        acc = self.evaluate(model, test_dataloader)
